// Use global fetch in Next.js environment, fallback to node-fetch if needed
...
class ManifestManager {
  ...
  async getManifestInfo() {
    try {
      const response = await fetchFunction(`${this.baseUrl}/Platform/Destiny2/Manifest/`, {
        headers: {
  ...
  async loadEssentialData(manifestInfo) {
    ...
    // For now, create structured sample data that matches Destiny 2's format
    // In production, you would fetch these from Bungie's database files
  ...
  createSampleItems() {
    // Sample armor and weapon definitions for development
    const items = {};
    ...
    const exoticWeapons = [
      ...
      },
      {
        ...
        itemType: 3, // Weapon
        itemSubType: 20, // Rocket Launcher
        tierType: 6, // Exotic
        inventory: { tierType: 6 },
        stats: { stats: {} }
      }

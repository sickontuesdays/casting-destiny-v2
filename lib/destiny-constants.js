// Updated Item Category Hashes based on actual Bungie API data
export const ITEM_CATEGORIES = {
  ...
  // Subclass Items - Updated with correct hashes
  SUBCLASS: [21],
  ASPECTS: [3124752623, 1469714392], // Multiple possible hashes for aspects
  FRAGMENTS: [3683254069, 1469714392], // Multiple possible hashes for fragments
  ...
  MODS: [59],
  ARMOR_MODS: [4104513227, 59], // Include general mod category
...
// Damage Types - Updated with correct mapping
...
export const UTILITY_FUNCTIONS = {
  ...
  // Get item class safely with improved logic
  getItemClass: (item) => {
  ...
  // Get item damage type safely with improved detection
  getDamageType: (item) => {
    // Check damageType property first (most reliable)
    if (item?.damageType !== undefined && item.damageType !== 0) {
      return DAMAGE_TYPES[item.damageType] || 'None';
    }
    
    // Check damageTypeHashes array
    if (item?.damageTypeHashes?.length > 0) {
      const damageHash = item.damageTypeHashes[0];
      return DAMAGE_TYPES[damageHash] || 'None';
    }
    
    // Check item name for damage type keywords (fallback)
...
// Enhanced category detection with multiple hash checking
export const CATEGORY_DETECTION = {
  ...
  // Enhanced detection for aspects - check multiple possible hashes and name patterns
  isAspect: (item) => {
    return UTILITY_FUNCTIONS.hasCategory(item, ITEM_CATEGORIES.ASPECTS) ||
           item?.displayProperties?.name?.toLowerCase().includes('aspect') ||
           item?.itemTypeDisplayName?.toLowerCase().includes('aspect');
  },
  
  // Enhanced detection for fragments - check multiple possible hashes and name patterns
  isFragment: (item) => {
    return UTILITY_FUNCTIONS.hasCategory(item, ITEM_CATEGORIES.FRAGMENTS) ||
           ...
           item?.itemTypeDisplayName?.toLowerCase().includes('fragment');
  },
  
  isGrenade: (item) => {
    return UTILITY_FUNCTIONS.hasCategory(item, ITEM_CATEGORIES.GRENADES) ||
           item?.displayProperties?.name?.toLowerCase().includes('grenade');
  },
  
  isMelee: (item) => {
    return UTILITY_FUNCTIONS.hasCategory(item, ITEM_CATEGORIES.MELEE_ABILITIES) ||
           item?.displayProperties?.name?.toLowerCase().match(/melee|punch|strike/);
  },
  
  isClassAbility: (item) => {
    return UTILITY_FUNCTIONS.hasCategory(item, ITEM_CATEGORIES.CLASS_ABILITIES) ||
           item?.displayProperties?.name?.toLowerCase().match(/barricade|dodge|rift/);
  },
  
  isSuper: (item) => {
    return UTILITY_FUNCTIONS.hasCategory(item, ITEM_CATEGORIES.SUPERS) ||
           item?.displayProperties?.name?.toLowerCase().match(/super|nova bomb|golden gun|fist of havoc|ward of dawn|well of radiance|shadowshot|hammer of sol/);
  },
  
  // Cosmetic items (to filter out) - enhanced detection
  isCosmetic: (item) => {

...
const filterBuildEssentialItems = (inventoryItems) => {
  ...
  Object.entries(inventoryItems).forEach(([hash, item]) => {
    ...
    if (UTILITY_FUNCTIONS.shouldExclude(item)) {
      counts.excluded++;
      return;
    }
  ...
  Object.values(items).forEach(item => {
    ...
    switch (item.buildReason) {
      ...
      case 'exotic_weapon':
        ...
        break;
      case 'mod':
        categorizeModByFunction(item, components.mods);
        break;
    }
  });
...
const organizeModData = (items) => {
  ...
  Object.values(items).forEach(item => {
    ...
    const itemSummary = createBuildItemSummary(item);
    categorizeModByFunction(item, mods);
  });

  return mods;
};

// Helper function to categorize mods by their actual function

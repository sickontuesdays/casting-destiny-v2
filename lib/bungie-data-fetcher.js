...
export const fetchAllDestinyData = async () => {
  try {
    ...
    // Try to fetch additional components, but don't fail if some are missing
    const componentPromises = [
      ...
      { name: 'talentGrids', promise: getManifestComponent('DestinyTalentGridDefinition').catch(e => { console.warn('Failed to fetch DestinyTalentGridDefinition:', e.message); return {}; }) },
    ...
    const organizedData = {
      ...
      abilities: organizeAbilities(manifest, talentGrids),

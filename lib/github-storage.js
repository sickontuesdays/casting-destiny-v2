...
class GitHubStorage {
  ...
  async loadManifest() {
    try {
      ...
      // Load manifest first
      const manifestData = await this.getFile(this.manifestPath)
      ...
      // Try to load metadata separately (optional)
      let metadata = null
      try {
        const metadataData = await this.getFile(this.metadataPath)
        metadata = metadataData?.content
      } catch (error) {
        console.log('No metadata found, continuing without it')
      }
  ...
  async saveManifest(manifest) {
    try {
      ...
      // Save files sequentially instead of in parallel to avoid conflicts
